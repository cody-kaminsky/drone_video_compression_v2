...
# Golden vectors for mb_header_dec_engine. The bits are written with the same
# syntax-element sequence src/encoder.c emits, and the expectation comes from
# dec_mb_header -- the routine the golden decoder itself uses. Neither side is
# a fresh reading of the spec.
mb_header_dec_vectors: $(BUILD)/mb_header_dec_vectors.txt
