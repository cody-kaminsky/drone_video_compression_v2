/* main.c — CLI driver for the HLS-port encoder.
 *
 * Identical CLI to the C reference's src/main.c, but invokes
 * encode_frame_h264_hls() so the test bench can compare the two.
 * Output format and stat lines are unchanged so tools/validate_byteexact.sh
...
int main(int argc, char **argv)
{
    if (argc < 5) {
        fprintf(stderr,
            ...
            "  HLS-port encoder. Bit-exact with C reference; line-buffered recon.\n",
            argv[0]);
        return 1;
    }
    ...
    if (read_file(in_path, frame, fr_size) != 0) { free(frame); return 4; }
    ...
    int rc = encode_frame_h264_hls(width, height, qp,
                                   frame,           width,
                                   frame + y_size,  width,
                                   recon,           width,
                                   recon + y_size,  width,
    ...
    if (recon_path) write_file(recon_path, recon, fr_size);
    if (bs_path && bs_buf && bs_len > 0) write_file(bs_path, bs_buf, (size_t)bs_len);
